...
// Backend URL එක මෙතන Variable එකක් ලෙස අර්ථ දක්වමු
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white p-4 md:p-8 lg:p-10 font-sans">
      <div className="max-w-7xl mx-auto">
        ...
        {/* ... (Analytics, Services, Messages, Reviews sections remain same but use API_BASE_URL) */}
        {activeTab === 'analytics' && (
            ...
            {reviews.map(r => (
              <div key={r._id} className={`bg-white/5 border p-6 rounded-3xl flex flex-col justify-between transition-all ${r.status === 'Pending' ? 'border-yellow-500/20 shadow-lg shadow-yellow-500/5' : 'border-white/10'}`}>
                ...
                <div className="flex gap-2">
                  {r.status === 'Pending' && (
                    <button onClick={() => updateReviewStatus(r._id, 'Approved')} className="flex-1 bg-blue-600 hover:bg-blue-500 text-white text-[9px] font-black uppercase py-2.5 rounded-xl transition-all">Approve</button>
                  )}
                  <button onClick={() => deleteItem('reviews', r._id)} className="px-4 py-2.5 bg-red-500/10 text-red-500 border border-red-500/20 rounded-xl hover:bg-red-500 hover:text-white transition-all text-[9px] font-black uppercase">Delete</button>
                </div>
              </div>
            ))}
          </div>
