...
// පියවර 1: 'https://your-backend.railway.app' වෙනුවට ඔයාගේ Railway URL එක මෙතනට දාන්න
...
const AdminDashboard = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      // මෙතනදී loading false කරනවා එතකොට error එකක් ආවත් screen එක හිරවෙලා තියෙන්නේ නැහැ
      setLoading(false); 
      if (err.response && (err.response.status === 401 || err.response.status === 403)) {
        handleLogout();
      } else {
        alert("Backend connection failed. Please check if the URL is correct.");
      }
    }
      ...
      <div className="max-w-7xl mx-auto">
        ...
        {activeTab === 'bookings' && (
          <div className="space-y-6 animate-fadeIn">
            ...
            <input type="text" placeholder="Search Vehicle Number..." className="w-full bg-white/5 border border-white/10 p-4 rounded-2xl text-sm focus:border-blue-500 outline-none transition-all" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
                    ...
                    {bookings.filter(b => b.vehicleNumber.toLowerCase().includes(searchTerm.toLowerCase())).map((b) => (
                      <tr key={b._id} className="hover:bg-white/[0.02] transition-colors">
                        ...
                        <td className="p-6 text-right space-x-2">
                           {b.status === 'Pending' && <button onClick={() => updateStatus(b._id, 'Approved')} className="p-2 bg-blue-500/10 text-blue-400 rounded-lg hover:bg-blue-500 transition-all">✓</button>}
                           {b.status === 'Approved' && <button onClick={() => updateStatus(b._id, 'Completed')} className="p-2 bg-green-500/10 text-green-400 rounded-lg hover:bg-green-500 transition-all">✓</button>}
                           <button onClick={() => deleteItem('bookings', b._id)} className="p-2 bg-red-500/10 text-red-400 rounded-lg hover:bg-red-500 transition-all">✕</button>
                        </td>
                      </tr>
                    ))}
                  </tbody>
...
const StatCard = ({ title, value, color }) => {
  const colors = { blue: 'text-blue-400 bg-blue-400/5', yellow: 'text-yellow-400 bg-yellow-400/5', indigo: 'text-indigo-400 bg-indigo-400/5', green: 'text-green-400 bg-green-400/5' };
  return (
    <div className={`bg-white/5 border border-white/10 p-6 rounded-3xl text-center transition-all hover:border-blue-500/50 ${colors[color]}`}>
